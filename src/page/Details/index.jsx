      ...
      setOrders(prevState => {
        ...
        if(exist) {
          const existDish = prevState.map(order => 
            ...
            {...order, quantity: order.quantity + dish.quantity} : order
          )
          ...
          setOneMore(0)
          const price = dish.strong ? String(dish.strong).replace("R$ ", "").replace(",", ".") : "0";
          const priceNumber = Number(price);
          setMessageError(`Prato adicionado para pedidos com o total de R$ ${(priceNumber * dish.quantity).toFixed(2)}. Obrigado!`);
          setIsLoaded(true)
          setIsModalOpen(true)
        }
      })
            ...
            />

                {
                  ...
                  <section className="details-mobile" key={dish.id}>
                      ...
                      <img src={`${api.defaults.baseURL}/files/${dish.avatar_dish}`}/>
                      ...
                      {user.role === USER_ROLE.CUSTOMER && (
                          <div>
                              <div className="add-decrease">
                                  <Minus onClick={() => setOneMore(oneMore === 0 ? oneMore : oneMore - 1)}/>
                                    {number}
                                  <Plus onClick={() => setOneMore(oneMore + 1)}/>
                              </div>
                              <Button 
                                icon={Receipt} 
