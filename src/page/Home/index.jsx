...
export function Home() {
    ...
    function handleAdd(newOrders) {
        setOrdwers((prevSatate) => {
            const exists = prevSatate.find((order) => order.span === newOrders.span);

            if (exists) {
                return prevSatate.map((order) =>
                    order.span === newOrders.span ?
                    {...order, quantity: order.quantity + newOrders.quantity}: order
                );
            }
    ...
    function handleFavorites(dish) {
      setFavorites(prevState => {
        const existFavorites = prevState.some(favorites => favorites.name === dish.name)

        let updateFavorites 

        if(existFavorites) {
          return prevState.filter(favorites => favorites.name !== dish.name)
        } else {
  ...
  const decreaseQuantity = (id) => {
    ...
    useEffect(() => {
      async function lookForDishes() {
        ...
        setDataDrinks(response.data.filter(response => response.category === "Bebidas"))
        setDataDesserts(response.data.filter(response => response.category === "Sobremesas"))
        setDataMeal(response.data.filter(response => response.category === "Refeição"))
    }
            ...
            <main>
                    ...
                    <div className="card-container" ref={mealsRef}>
                    {
                      dataMeal && dataMeal.map(meal => (
                        <Card
                            ...
                            handleAddFavorite={() => handleFavorites({
                              img: meal.avatar_dish,
                              name: meal.name
                            })}
                            leastOne={() => decreaseQuantity(meal.id)}
                            oneMore={() => increaseQuantity(meal.id)}
                            setNumber={setNumber}
                            handleCardClick={() => handleCardClick(meal.id)}
                            onClick={() =>
                                handleAdd({
                                    id: meal.id,
                                    img: meal.avatar_dish,
                ...
                <section className="desserts">
                    ...
                    <div className="card-container" ref={dessertsRef}>
                      {
                        dataDesserts && dataDesserts.map(desserts => (
                          <Card
                              key={desserts.id}
                              ...
                              leastOne={() => decreaseQuantity(desserts.id)}
                              oneMore={() => increaseQuantity(desserts.id)}
                              handleAddFavorite={() => handleFavorites({
                                img: desserts.avatar_dish,
                                name: desserts.name
                              })}
                              handleCardClick={() => handleCardClick(desserts.id)}
                              id={desserts.id}
                              alt={desserts.name}
                              onClick={() =>
                                handleAdd({
                                    ...
                                    strong: desserts.price,
                                    description: desserts.description,
                                    quantity: quantities[desserts.id],
                                })
                            }
                          />
                        ))
                ...
                <section className="drinks">
                    ...
                    {
                      dataDrinks && dataDrinks.map(drinks => (
                        <Card
                            key={drinks.id}
                            ...
                            leastOne={() => decreaseQuantity(drinks.id)}
                            oneMore={() => increaseQuantity(drinks.id)}
                            handleAddFavorite={() => handleFavorites({
                              img: drinks.avatar_dish,
                              name: drinks.name
                            })}
                            handleCardClick={() => handleCardClick(drinks.id)}
                            onClick={() =>
                              handleAdd({
                                  id: drinks.id,
                                  img: drinks.avatar_dish,
